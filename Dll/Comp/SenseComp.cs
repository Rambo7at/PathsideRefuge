...
public partial class SenseComp : Area3D
{
	...
	{
		// 检测生物
		PerformDetection(m_CreatureList, m_DetectedCreaturesList); // 只检测 Layer 2
																   // 检测物品
		PerformDetection(m_ItemList, m_DetectedItemList);         // 只检测 Layer 3
	}
	...
	private void PerformDetection<[MustBeVariant] T>(Array<T> list1, Array<T> list2)
	{
		...
		foreach (var obj in list1)
		{
			...
			{
				node3d = obj as CreatureBase;
				mask = 2;

			}
			else if (obj is ItemComp)
			{
				node3d = obj as ItemComp;
				mask = 3;
			}

			if (node3d == null) continue;

			var result = PerformRaycast(m_Eye.GlobalPosition, node3d.GlobalPosition, m_CreatureBase.m_SelfExclude, mask);

			if (result?.Count > 0 && result.TryGetValue("collider", out var value))
			{

				var detected = value.As<Node3D>();
	...
	private Dictionary PerformRaycast(Vector3 from, Vector3 to, Array<Rid> exclude, uint mask = 0)
	{
		...
		m_CreatureBase.SetPhysicsRay(from, to, exclude, mask);
